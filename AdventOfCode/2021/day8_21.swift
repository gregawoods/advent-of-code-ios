...
struct Y21Day8: DayProtocol {
    ...
    func part2(_ input: [String]) -> Int {
        ...
        for row in input {
            ...
            // done.... ?
            sum += Int(results.map { String($0) }.joined())!
        }

        return sum
    }

    func combineSegments(_ template: [Piece: Character], _ segments: [Piece]) -> String {
        return segments.map { String(template[$0]!) }.sorted().joined()
    }

    func translate(str: String, segments: [Piece: Character]) -> Int {
        let keys = [
            ...
            combineSegments(segments, Piece.allCases),                                         // eight
